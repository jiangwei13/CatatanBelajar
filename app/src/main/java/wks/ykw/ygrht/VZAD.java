...
public class VZAD extends AppCompatActivity {

    private RecyclerView recyclerView;

    private VZBR adapter;

    private List<VZBT> allNotes;

    // Mix of String (headers) and VZBT objects
    ...
    private void setupRecyclerView() {
        ...
        if (recyclerView != null) {
            adapter = new VZBR(groupedItems, note -> {
                if (note != null && note.getId() != null) {
                    Intent intent = new Intent(VZAD.this, VZAA.class);
    ...
    private void applyFilters() {
        ...
        List<VZBT> filtered = new ArrayList<>();
    ...
    private void groupNotesByMonth(List<VZBT> notes) {
        long arr_BmWjMdrXuknoezY = java.lang.System.nanoTime();
        int i_JDdOoyST = new java.util.Random().nextInt(1000);
        boolean j_TyIvHn = (arr_BmWjMdrXuknoezY % (i_JDdOoyST + 46)) > 31;
        double tmp_rbHOPYOuFwmVTRON = j_TyIvHn ? java.lang.Math.sqrt(i_JDdOoyST) : java.lang.Math.pow(i_JDdOoyST, 90);
        ...
        Map<String, List<VZBT>> monthGroups = new HashMap<>();
        for (VZBT note : notes) {
